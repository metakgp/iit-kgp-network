...
function installQuestions() {
    echo "Welcome to the Stunnel + OpenVPN installer by Lagnos (improvised over angristan)!"
    echo "Use the VPN at your own risk"
    echo ""
...
function installOpenVPN() {
    ...
    # Lagnos : port 443 will be used by stunnel
...
server 10.8.0.0 255.255.255.0
    ...
    # Lagnos : this needs to be there for passing stunnel packets seperately otherwise
    # stunnel packets will also route through the tunnel and won't work
    echo "push \"route $IP 255.255.255.255 net_gateway\"" >>/etc/openvpn/server.conf
...
connect = 0.0.0.0:1194
    ...
    # Systemctl service to run before openvpn service
    echo "[Unit]
...
[Service]
Type=simple
ExecStart=/usr/bin/stunnel /etc/stunnel/server.conf
RemainAfterExit=yes

[Install]
    WantedBy=multi-user.target" >/etc/systemd/system/stunnel-openvpn.service
    
    # Enable service and apply rules
    systemctl daemon-reload
    systemctl enable stunnel-openvpn
    systemctl start stunnel-openvpn
...
function removeOpenVPN() {
    ...
    if [[ $REMOVE == 'y' ]]; then
        ...
        # Lagnos : Stop the stunnel service
        systemctl disable stunnel-openvpn
        systemctl stop stunnel-openvpn
        ...
        rm /etc/systemd/system/stunnel-openvpn.service
        systemctl daemon-reload
